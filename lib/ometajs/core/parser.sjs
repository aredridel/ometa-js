...
//
AbstractParser.prototype._cache = function (grammar, rule, body) {
...
//
AbstractParser.prototype._list = function list(body, flat, _) {
  var current = this._current(_),
      res;
...
//
AbstractParser.prototype._match = function match(value, _) {
  if (this._current(_) === value) {
    return this._skip(_);
  } else {
    return false;
  }
...
//
AbstractParser.prototype._fnMatch = function match(fn, result, _) {
  if (fn.call(this, this._current(_))) {
    return this._skip(null, result, _);
  } else {
    return false;
  }
};
...
//
AbstractParser.prototype._current = function current(_) {
...
//
AbstractParser.prototype._isEnd = function isEnd(_) {
...
//
AbstractParser.prototype._skip = function skip(num, result, _) {
  if (this._type === 'simulate') {
    this._source.skip(num);
  } else {
    if (num) this._offset += num - 1;
    this._setIntermediate(this._current(_), result);
    this._offset++;
...
//
Simulate.prototype.skip = function skip(num, _) {
